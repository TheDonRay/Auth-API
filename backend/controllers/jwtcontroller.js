...
const jwtFunction = (req, res) => {
  ...
  if (userExists) {
    ...
    res.json({ accessToken: accessToken }); // what this does now is basically once username is validated we create a token that holds a payload value which is the users username and we create a JWT token for that information that we can generate.
  }
};
